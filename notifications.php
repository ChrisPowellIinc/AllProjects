...
mysqli_query($con, "UPDATE users SET notescheck=now() WHERE username='$log_username' LIMIT 1");
?><?php
$friend_requests = "";
...
if($numrows < 1){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$thumbquery = mysqli_query($con, "SELECT avatar FROM users WHERE username='$user1' LIMIT 1");
		...
		if($user1avatar == NULL){
			$user1pic = '<img src="Icons/usericon.jpg" alt="'.$user1.'" class="user_pic">';
		}
...
function friendReqHandler(action,reqid,user1,SearchFeild){
	var conf = confirm("Press OK to '"+action+"' this friend request.");
	if(conf != true){
		return false;
	}
	_(SearchFeild).innerHTML = "processing ...";
	var ajax = ajaxObj("POST", "php_parsers/friend_system.php");
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "accept_ok"){
				_(SearchFeild).innerHTML = "<b>Request Accepted!</b><br />Your are now friends";
			} else if(ajax.responseText == "reject_ok"){
				_(SearchFeild).innerHTML = "<b>Request Rejected</b><br />You chose to reject friendship with this user";
			} else {
				_(SearchFeild).innerHTML = ajax.responseText;
			}
		}
	}
