...
<head>
<meta charset="UTF-8">
<title>Lilli Pilli Health Bar- All Of Us</title>
...
div.spinnable-carousel.cover div:nth-child(1) {
	...
	background-image: url(php_includes/orgs/lillipilli/cov1.jpg);
}
div.spinnable-carousel.cover div:nth-child(2) {
	background-size: cover;
	background-color: #0F495C;
	background-image: url(php_includes/orgs/lillipilli/cov2.jpg);
	opacity: .95;
	background-position: center center;
}
...
div.spinnable-carousel.cover div:nth-child(3) {
	background-size: cover;
	background-color: #0F495C;
	background-image: url(php_includes/orgs/lillipilli/cov3.jpg);
	opacity: .95;
	background-position: center center;
}
div.spinnable-carousel.cover div:nth-child(4) {
	background-size: cover;
	background-color: #0F495C;
	background-image: url(php_includes/orgs/lillipilli/cov4.jpg);
	opacity: .95;
	background-position: center center;
}
div.spinnable-carousel.cover div:nth-child(5) {
	background-size: cover;
	background-color: #0F495C;
	background-image: url(php_includes/orgs/lillipilli/cov5.jpg);
	opacity: .95;
	background-position: center center;
}
div.spinnable-carousel.cover div:nth-child(6) {
	background-size: cover;
	background-color: #0F495C;
	background-image: url(php_includes/orgs/lillipilli/cov6.jpg);
	opacity: .95;
	background-position: center center;
}
...
<link rel="stylesheet" href="css/user.css" type="t/css">
...
<script>
	...
	function yScroll(){
		...
		if(yPos > 90){
			...
			document.getElementById('profileimage').className = "covercollapse";
...
<script type="text/javascript">
function friendToggle(type,user,SearchFeild){
	var conf = confirm("Press OK to confirm the '"+type+"' action for user <?php echo $u; ?>.");
	if(conf != true){
		return false;
	}
	_(SearchFeild).innerHTML = 'please wait ...';
	var ajax = ajaxObj("POST", "php_parsers/friend_system.php");
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "friend_request_sent"){
				_(SearchFeild).innerHTML = 'OK Friend Request Sent';
			} else if(ajax.responseText == "unfriend_ok"){
				_(SearchFeild).innerHTML = '<button onclick="friendToggle(\'friend\',\'<?php echo $u; ?>\',\'friendBtn\')">Request As Friend</button>';
			} else {
				alert(ajax.responseText);
				_(SearchFeild).innerHTML = 'Try again later';
			}
		}
	}
	ajax.send("type="+type+"&user="+user);
}
function blockToggle(type,blockee,SearchFeild){
	...
	SearchFeild.innerHTML = 'please wait ...';
	var ajax = ajaxObj("POST", "php_parsers/block_system.php");
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			if(ajax.responseText == "blocked_ok"){
				SearchFeild.innerHTML = '<button onclick="blockToggle(\'unblock\',\'<?php echo $u; ?>\',\'blockBtn\')">Unblock User</button>';
			} else if(ajax.responseText == "unblocked_ok"){
				SearchFeild.innerHTML = '<button onclick="blockToggle(\'block\',\'<?php echo $u; ?>\',\'blockBtn\')">Block User</button>';
			} else {
				alert(ajax.responseText);
				SearchFeild.innerHTML = 'Try again later';
			}
		}
	}
...
<div id="ExpandCoverButton"></div>
      <div id="ExpandFeatureButton" class="ExpandFeatureStage1">
  			<div id="postcontainerlist" class="postcontainerlistclosed">           
            	<span class="detailsfontsnums" style="width: 98%;">Posts: <span class="detailsfontsnumslog">2015</span></span>
                <div id="containerpostspec" class="postspectect">My first post! How's it looking?</div>
                <div id="containerpostspec" class="postspecimg"></div>
                <div id="containerpostspec" class="postspecimg2"></div>
                <div id="containerpostspec" class="postspecimg3"></div>
            </div> 
  	</div>
  ...
  <div id="UserBoxTopLeft" class="UsernameNarrow"><?php echo $u; ?></div>
  <div id="StatsInfo">		
  			<iframe src="http://www.westchesterluxuries.com/Clients/Oconnors/001.html" width="100%" height="100%" frameborder="0"></iframe>
	</div>
  <div id="Adboxbox">
  	<div id="arrowpos" class="uparrow"></div>
