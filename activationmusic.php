...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	$sql = "SELECT * FROM usersmusic WHERE musicid='$id' AND musicname='$u' AND email='$e' AND password='$p' LIMIT 1";
	...
	$sql = "UPDATE usersmusic SET activated='1' WHERE musicid='$id' LIMIT 1";
    $query = mysqli_query($con, $sql);
	// Optional double check to see if activated in fact now = 1
	$sql = "SELECT * FROM usersmusic WHERE musicid='$id' AND activated='1' LIMIT 1";
    $query = mysqli_query($con, $sql);
	$numrows = mysqli_num_rows($query);
